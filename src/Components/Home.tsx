...
const Home = (props: any) => {
    ...
    const [isAnimationsClicked, setIsAnimationsClicked] = useState<boolean>(false);
        ...
        <>
            {!isAnimationsClicked && !isCreateAnimationClicked && (
                ...
                <div className="homeButtonsContainer">
                    <button style={{fontSize: 20 * getSize()}} className="homeButtons" onClick={() => setIsCreateAnimationClicked(true)}>
                        Create your own animation
                    </button>
                    <button style={{fontSize: 20 * getSize(), marginTop: "5%"}} className="homeButtons" onClick={() => setIsAnimationsClicked(true)}>
                        Animations from the community
                    </button>
                </div>
                </>
            )}
            {isCreateAnimationClicked && (
                <SpikeStrat app={props.app}/>
            )}
            {isAnimationsClicked && (
                <CommunityAnimations app={props.app}/>
            )}
        </>
    )
}
